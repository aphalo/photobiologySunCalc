...
The functions and methods described in this User Guide return either values that
...
instant when the center of the solar disk is at the theoretical (= astronomical) horizon.
...
Two names match twilight angles that are approximately corrected for atmospheric refraction, based on assumed atmosphere composition including water vapour column. Refraction alters the path of the direct sunlight beam, making the times of apparent (= visual) sunset slightly different from the astronomical (= real) position of the sun.
...
Twilight is also relevant to the computation of day length and night length. The default is to use the centre of the sun disk, but this can be changed. For the 
...
Although the normal global short-wave irradiance at the top of the atmosphere
varies so little as to allow it to be described as the _solar constant_, the
value of the extraterrestrial irradiance on a plane parallel to the ground
depends strongly on the position of the sun and weakly on changes in the Sun to
Earth distance. This is a simple trigonometric computation as direct radiation
is the overwhelming component of sunlight above the atmosphere, when light
reflected by the Moon and planets is ignored as done here.
